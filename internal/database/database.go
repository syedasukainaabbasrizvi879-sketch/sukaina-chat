...
func SaveMessage(senderID, recipientID, content string) error {
	query := `INSERT INTO messages (sender_id, recipient_id, content) VALUES ($1::UUID, $2::UUID, $3)`
	_, err := DB.Exec(query, senderID, recipientID, content)
	if err != nil {
		log.Printf("❌ Failed to save message in DB: %v", err)
	}
	return err
}

func GetMessages(userID string) ([]models.Message, error) {
	query := `SELECT message_id, sender_id, recipient_id, content, created_at FROM messages WHERE recipient_id = $1::UUID OR sender_id = $1::UUID ORDER BY created_at ASC LIMIT 100`
